...
class WebcamClassifier {
  ...
  animate() {
    if (this.isDown) {
      if (this.current.imagesCount > MAX_IMAGES_PER_CLASS){
        alert("Max Number of Images this class can have reached!");
        this.isDown = false;
      ...
      const computeConfidences = () => {
        ...
        for (let index = 0; index < CLASS_COUNT; index += 1) {
          ...
          const softMaxProb = Math.exp(classTopKMap[index]);
          sumOfProbs += softMaxProb;
          confidences[index] = probability;
          soft[index] = softMaxProb;
        }

        for (let index = 0; index < soft.length; index += 1){
          soft[index] = soft[index] / sumOfProbs;
        }



	      // Change the two NULLs below to get the global values that can be used in onSpecialButtonClick
        // Confidence displayed is the original confidence. Scaled up confidence is shown in the
